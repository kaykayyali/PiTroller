...
Xbox_Interpreter.prototype.handle_y_button = function(event) {
	...
	Requester(thiurl, function(error, response, data) {
		if (error) {
			console.warn(error);
		}
		else {
...
Xbox_Interpreter.prototype.handle_lb_button = function(event) {
	var url = this.pibot_url + "/smell_off";
	console.log("Handling ", this.button_map[event.number].name , " event.", event);
	console.log("SENDING TO ", url);
	Requester(turl, function(error, response, data) {
		if (error) {
			console.warn(error);
		}
		else {
...
Xbox_Interpreter.prototype.handle_rb_button = function(event) {
	var url = this.pibot_url + "/smell_on";
	console.log("Handling ", this.button_map[event.number].name , " event.", event);
	console.log("SENDING TO ", url);
	Requester(url, function(error, response) {
		if (error) {
			console.warn(error);
		}
		else {
