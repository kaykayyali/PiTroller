...
// R3 X-Axis (Cam Pan)
Xbox_Interpreter.prototype.handle_rx_axis = function(event) {
	var url = this.pibot_url;
	var servo_id = this.servos["pan"].id;
	if (event.value < 0) {
		url += '/move_servo/' + servo_id + '/1';
	}
	else if (event.value > 0) {
		url += '/move_servo/' + servo_id + '/-1';
	}
...
// R3 Y-Axis (Cam Tilt)
Xbox_Interpreter.prototype.handle_ry_axis = function(event) {
	var url = this.pibot_url;
	var servo_id = this.servos["tilt"].id;
	if (event.value > 0) {
		url += '/move_servo/' + servo_id + '/1';
	}
	else if (event.value < 0) {
...
// RT Trigger (Claw)
Xbox_Interpreter.prototype.handle_rt_axis = function(event) {
	var url = this.pibot_url;
	var servo_id = this.servos["claw"].id;
	if (event.value > 0) {
		url += '/move_servo/' + servo_id + '/1';
	}
	else if (event.value < 0) {
...
// LT Trigger (Claw)
Xbox_Interpreter.prototype.handle_lt_axis = function(event) {
	var url = this.pibot_url;
	var servo_id = this.servos["claw"].id;
	if (event.value < 0) {
		url += '/move_servo/' + servo_id + '/1';
